...
for c in chunks:
    ...
    chunk_embeddings_full.append({"text": c, "embedding": result.embeddings[0].values})
...
accuracy_top1 = evaluate_retrieval(eval_set, chunks, chunk_embeddings_full, top_n=1)
print(f"Top-1 accuracy: {accuracy_top1 * 100:.0f}%")

accuracy_top3 = evaluate_retrieval(eval_set, chunks, chunk_embeddings_full, top_n=3)
print(f"Top-3 accuracy: {accuracy_top3 * 100:.0f}%")

# accuracy_full = evaluate_retrieval(eval_set, chunks, chunk_embeddings_full, top_n=1)
# print(f"Top-1 accuracy, full dimensions: {accuracy_full * 100:.0f}%")
...
(.venv) barji@Barji:~/projects/agentic-ai-journey/day15$ python3 rag_project.py
...
FAIL | How old are China's ancient fortifications?
   expected: The Great Wall of China is a series of fortifications built 
   got:      Mount Everest is the tallest mountain above sea level on Ear
FAIL | Which structure took centuries to build?
   expected: The Great Wall of China is a series of fortifications built 
   got:      The Eiffel Tower was completed in 1889 for the World's Fair 
Top-1 accuracy: 60%
PASS | How tall is the tallest mountain on Earth?
PASS | What year was a famous Parisian landmark completed?
PASS | How much oxygen does a rainforest produce?
