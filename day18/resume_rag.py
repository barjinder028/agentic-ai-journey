...
eval_set = [
    {"question": "Does Barjinder know Python?", "expected_chunk_indices": [0, 1]},
    {"question": "What did Barjinder do at Amazon?", "expected_chunk_indices": [12]},
]
...
def evaluate_retrieval(eval_set, chunks, chunk_embeddings, top_n=1):
    ...
    for item in eval_set:
        ...
        expected_texts = [chunks[i] for i in item["expected_chunk_indices"]]

        is_correct = any(text in retrieved_texts for text in expected_texts)
        if is_correct:
            correct += 1
...
results = search("What work experience does the candidate have from 2020 to 2025?", chunk_embeddings, top_n=5)
for r in results:
    print(round(r["score"], 3), "-", r["text"][:70])

accuracy = evaluate_retrieval(eval_set, chunks, chunk_embeddings, top_n=1)
print(f"Top-1 accuracy on real document: {accuracy * 100:.0f}%")


accuracy_top3 = evaluate_retrieval(eval_set, chunks, chunk_embeddings, top_n=3)
...
0.684 - PROFESSIONAL EXPERIENCE:
Senior Cloud Engineer - AI Quality Evaluation
0.666 - Quality Auditor - QA on ML-Integrated Systems          Jan 2020 - May 
0.615 - SUMMARY:
Agentic AI Engineer with a hands-on foundation in Python, LLM
0.586 - RAG Pipeline with Evaluation Harness                   (Python · Embed
0.585 - Senior Data Associate - Quality & Data Operations      Jun 2018 - Dec 
